...
function applyMove(san,fen){
  ...
  if(cursor.next && cursor.next.san===san){
    ...
    if(cursor.next){
      // there is already a mainline
      if(chess.turn()==="b"){
        // inserting BLACK alternative → promote to mainline
...
function renderSeq(n,container,moveNo,side){
  ...
  while(cur){
    ...
    if(s==="w"){
      if(cur.next){cur=cur.next;s="b";continue;}
...
function appendMove(container,node){
  const ref=node;
  const span=document.createElement("span");
  span.className="move"+(ref===cursor?" active":"");
  span.textContent=figSAN(ref.san);
  span.onclick=()=>{
    cursor=ref;
    rebuildTo(ref,true);
    render();
